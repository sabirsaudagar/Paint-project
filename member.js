...
///***************** *////
// This declares an asynchronous function named getCart. Async
//  functions are used when you need to work with asynchronous operations,
//  such as fetching data from a database.


async function getCart() 
{
///Promice are used to handle ansycronized method
 //The resolve function is a callback that is used to fulfill 
 // the promise when the asynchronous operation is successful.
   
        return new Promise(async(resolve)=>{
        if (loggedIn) {
            const cartSnapshot = await getDocs(collection(firestore, 'users', auth.currentUser.uid, 'cart'))
            ///If the user is logged in, it uses Firebase Firestore to get a snapshot of the user's cart. 
            /// The await keyword is used here to wait for the asynchronous operation to complete.
          
          ///If the cart is empty, it resolves the promise with an empty array.
            if (cartSnapshot.empty) 
            {
                resolve([])
            }

            // If the cart is not empty, it uses the forEach 
            // method to iterate over each document in the cart 
            let cart = []
            // For Each method is used to take a specific action.
        ...
        //If the user is not logged in, it retrieves the cart data 
        // from the session storage and processes it similarly to the logged-in case.
        else {
            ...
            // This resolves the promise with the final cart array, whether the user is logged in or not.
